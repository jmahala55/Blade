class Conninfo {
    constructor(parentId) {
        ...
        // Lazy-load pretty-bytes (ESM only)
        import("pretty-bytes").then(pb => {
            this._pb = pb.default;
        });

        // Smoothie works fine with require
        const { TimeSeries, SmoothieChart } = require("smoothie/smoothie.js");
    ...
    async updateInfo() {
        ...
        if (window.mods.netstat.offline || !window.mods.netstat.iface) {
        ...
        let data = await window.si.networkStats(window.mods.netstat.iface);

        let max0 = this.series[0].maxValue;
        let max1 = -this.series[1].minValue;
        if (max0 > max1) this.series[1].minValue = -max0;
        else if (max1 > max0) this.series[0].maxValue = max1;
        ...
        this.current.innerText = "UP " + (data[0].tx_sec / 125000).toFixed(2) + " DOWN " + (data[0].rx_sec / 125000).toFixed(2);
    }
}
